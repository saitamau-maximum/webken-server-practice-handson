...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", () => showPostModal("create"));
...
async function savePost() {
  ...
  const categoryId =
    parseInt(document.getElementById("post-category").value) || null;
  const tags = document
  ...
  try {
    ...
    const url = isEdit
      ? `http://localhost:3000/api/posts/${currentPost.id}`
...
async function addComment() {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/api/posts/${currentPost.id}/comments`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
